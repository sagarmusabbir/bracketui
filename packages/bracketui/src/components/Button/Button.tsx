...
const Button = forwardRef(function Button<T extends ElementType = "button">(
  ...
  const variantButton = outline
    ...
    : "border-none bg-gray-950 dark:bg-gray-100 text-white dark:text-gray-950 [@media(hover:hover)]:hover:opacity-90 active:opacity-80";

  // Size classes with fixed heights
  const sizeClasses = clsx({
    "px-3 h-9 text-xs rounded-md": size === "sm",
    "px-4 h-10 text-sm rounded-md": size === "md",
